...
export const ImpresorasPage = () => {
  ...
  const saveConfig = async (type, printerName) => {
    ...
    try {
      const bodyStr = `id_usuario=${encodeURIComponent(uid)}&tipo=${encodeURIComponent(type)}&impresora=${encodeURIComponent(printerName)}`;
    ...
    <div className="absolute inset-0 overflow-y-auto bg-slate-50 p-4 sm:p-6 lg:p-8">
      <div className="max-w-6xl mx-auto space-y-6 pb-32">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="flex items-center gap-5 px-8 py-5 border-b border-slate-100">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${connected ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-500'}`}>
              <i className={`fas ${connected ? 'fa-check-circle' : 'fa-times-circle'}`} />
            </div>
            <div className="flex-1">
              <h1 className="text-xl font-extrabold text-slate-800 tracking-tight">Configuraci&oacute;n de Impresoras</h1>
              <p className={`text-sm font-semibold mt-0.5 ${connected ? 'text-emerald-600' : 'text-red-500'}`}>
                {connected ? 'SERVICIO ACTIVO' : 'SERVICIO INACTIVO'}
              </p>
            </div>
            <div className="flex flex-col gap-2">
              <button onClick={() => scanPrinters(true)} disabled={scanning || loadingQZ}
              ...
              <a href="/qz-tray-2.2.5-x86_64.exe"
                className="px-5 h-8 border border-slate-200 text-slate-600 rounded-lg text-[10px] font-bold hover:bg-slate-50 transition-all flex items-center gap-2">
              ...
              <button onClick={() => {
                ...
                className="px-5 h-8 border border-emerald-200 text-emerald-700 rounded-lg text-[10px] font-bold hover:bg-emerald-50 transition-all flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-8">
            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
              ...
              <button onClick={() => testPrint(printerBoletos, 'TICKET DE PRUEBA')} disabled={testing || !printerBoletos}
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
              ...
              <button onClick={() => testPrint(printerGuias, 'GUIA DE PRUEBA')} disabled={testing || !printerGuias}
          ...
          {/* MÉTODO DE IMPRESIÓN */}
          <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm mx-8 mb-8">
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="text-sm font-extrabold text-slate-700 uppercase tracking-wider">M&eacute;todo de Impresi&oacute;n</h2>
            </div>
            <p className="text-xs text-slate-500 mb-5 leading-relaxed">
              Seleccione c&oacute;mo desea imprimir los boletos despu&eacute;s de emitirlos.
            </p>
            <div className="flex gap-6">
              <label className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all flex-1 ${
                ...
                />
                <div>
                  <div className="font-bold text-sm text-slate-800">Manual</div>
                  <div className="text-xs text-slate-500 mt-0.5">Abre el ticket en un modal para imprimir con el navegador</div>
                </div>
              </label>
              <label className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all flex-1 ${
                ...
                />
                <div>
                  <div className="font-bold text-sm text-slate-800">Directa</div>
                  <div className="text-xs text-slate-500 mt-0.5">Imprime directamente en la impresora t&eacute;rmica v&iacute;a QZ Tray sin mostrar modal</div>
                </div>
              </label>
            </div>
          </div>
