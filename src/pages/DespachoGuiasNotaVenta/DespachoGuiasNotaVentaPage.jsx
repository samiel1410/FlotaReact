...
import { despachoNotaVentaService } from '../../services/despachoNotaVenta.service';
...
const DespachoRow = memo(({ despacho, onPdf, onEditar, onAgregarGuiaClick, formatDate, renderEstado }) => (
  <tr className="hover:bg-slate-50/70 transition-colors">
    <td className="px-4 py-3 text-sm font-bold text-slate-700">
      {despacho.numero_despacho_maestro_nota_venta || '-'}
    </td>
    <td className="px-4 py-3 text-sm text-slate-600">
      {formatDate(despacho.fecha_despacho_maestro_nota_venta)}
    </td>
    ...
    <td className="px-4 py-3 text-center">
      {renderEstado(despacho.estado_despacho_maestro_nota_venta)}
    </td>
...
const DetalleRow = memo(({ detalle, onQuitarGuia }) => (
  <tr className="hover:bg-slate-50">
    ...
    <td className="px-4 py-2 text-sm">
      {String(detalle.estado_despacho_detalle_nota_venta) === '1'
    ...
    <td className="px-4 py-2 text-center">
      <button onClick={() => onQuitarGuia(detalle.id_despacho_detalle_nota_venta)} className="px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white rounded-lg text-xs font-bold transition-all" title="Quitar guía del despacho">
...
export const DespachoGuiasNotaVentaPage = () => {
  // ─── Estado Principal ───
  ...
  const cargarDespachos = useCallback(async (pageNum = 1, filtrosActuales = null) => {
    ...
    try {
      ...
      const res = await despachoNotaVentaService.listar(params);
      if (res.success) {
  ...
  const handleEditarDespacho = async (despacho) => {
    try {
      const res = await despachoNotaVentaService.verificarGuia({ id_despacho_maestro: despacho.id_despacho_maestro_nota_venta });
      if (res.success) {
        if (res.tipo === 0) {
          if (String(despacho.estado_despacho_maestro_nota_venta) === '1') {
            setSelectedDespacho(despacho);
            setEditarBusId(despacho.id_fkbus_despacho_maestro_nota_venta || '');
            setEditarBusNombre(despacho.nombre_bus || '');
            setShowEditarModal(true);
            // Load details
            cargarDetalles(despacho.id_despacho_maestro_nota_venta);
          } else {
  ...
  const cargarDetalles = async (idMaestro) => {
    try {
      ...
      const res = await despachoNotaVentaService.listarDetalle(params);
      if (res.success) {
  ...
  const handleGuardarEdicion = async () => {
    ...
    try {
      const res = await despachoNotaVentaService.editar({
        id_maestro: selectedDespacho.id_despacho_maestro_nota_venta,
        id_bus: editarBusId,
  ...
  const handleQuitarGuia = async (idDetalle) => {
    ...
    try {
      const res = await despachoNotaVentaService.eliminar({
        id_despacho_detalle: idDetalle,
        motivo: 'Eliminado por usuario'
      });
      if (res.success) {
        toast.success('Guía eliminada del despacho');
        cargarDetalles(selectedDespacho?.id_despacho_maestro_nota_venta);
        cargarDespachos(page);
      } else {
  ...
  const agregarGuia = async (guia) => {
    try {
      const res = await despachoNotaVentaService.agregarGuia({
        id_guia: guia.id_guia,
        id_despacho_maestro: selectedDespacho.id_despacho_maestro_nota_venta,
        destino_guia: guia.destino_guia || selectedDespacho.nombre_destino || '',
        bus: guia.bus || selectedDespacho.id_fkbus_despacho_maestro_nota_venta || '',
        nombre_oficina: selectedDespacho.nombre_destino || ''
      });
      if (res.success) {
        if (res.tipo === 1) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50/50">
      ...
      <div className="bg-white border-b border-slate-200 shadow-sm shrink-0">
        <div className="flex items-center justify-between px-4 py-2 bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-black text-slate-800 uppercase tracking-tight leading-none">Despacho de Notas de Venta</h1>
      ...
      <div className="p-4">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-100">
                  {despachos.map((d, idx) => (
                    <DespachoRow
                      key={d.id_despacho_maestro_nota_venta || idx}
                      despacho={d}
                      onPdf={handlePdf}
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col">
              <div className="bg-slate-800 px-6 py-4 flex justify-between items-center shrink-0">
                <h3 className="text-sm font-bold text-white">
                  ...
                  DESPACHO N° {selectedDespacho.numero_despacho_maestro_nota_venta}
                </h3>
                      ...
                      <table className="w-full">
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {detalles.map((det, idx) => (
                            <DetalleRow
                              key={det.id_despacho_detalle_nota_venta || idx}
                              detalle={det}
        ...
        {showNuevoModal && (
          <NuevoDespachoGuiaNotaVentaModal
        ...
        {showBusquedaModal && selectedDespacho && (
          <BusquedaGuiaDespachoNotaVentaModal
            idDespachoMaestro={selectedDespacho.id_despacho_maestro_nota_venta}
            bus={selectedDespacho.id_fkbus_despacho_maestro_nota_venta}
        ...
        <PdfViewerModal
          ...
          url={pdfUrl}
          title="Despacho de Notas de Venta"
...
export default DespachoGuiasNotaVentaPage;
