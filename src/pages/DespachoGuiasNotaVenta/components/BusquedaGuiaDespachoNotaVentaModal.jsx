...
export const BusquedaGuiaDespachoNotaVentaModal = memo(({ bus, onClose, onSelect }) => {
  ...
  const cargarGuias = useCallback(async (pageNum, numeroGuia) => {
    setLoading(true);
    try {
      const params = {
      ...
      const res = await despachoNotaVentaService.listarGuiasDisponibles(params);
      if (res.success) {
        setGuias(res.data || []);
