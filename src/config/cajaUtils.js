...
import AperturaCajaForm from '../pages/CajaBoleteria/components/AperturaCajaForm';
import CierreCajaForm from '../pages/CajaBoleteria/components/CierreCajaForm';
...
export const createAperturaAction = (endpointUrl) => {
  return {
    ...
    handler: (ctx) => {
      // Usamos el ctx para abrir el custom modal con el componente react
...
export const createBuscarCajaAction = () => {
  return {
    ...
    color: 'bg-emerald-600 hover:bg-emerald-700 text-white',
    handler: ({ refreshList }) => {
      // El parámetro apertura=1 le indica al backend que filtre por el usuario autenticado en el JWT
    ...
    handler: (row, ctx) => {
      if (ctx && ctx.openCustomModal) {
        ctx.openCustomModal(
          ...
          `Cierre Caja #${row.numero_caja || row.id_caja || ''}`
        );
      }
    }
  };
