 ...
 */
export const GuiaNotaVentaService = {
  
  /**
   * Obtiene la lista de guías con paginación y filtros
   */
  getGuias: async (params = {}) => {
    const response = await api.get('/guia_nota_venta/guialistado', { params });
  ...
  buscarGuiaPorId: async (idGuia) => {
    const response = await api.get('/guia_nota_venta/buscarGuiaId', { params: { id_guia: idGuia } });
    return response.data;
  },
  ...
  anularGuia: async (idGuia, idUsuario, motivo) => {
    const response = await api.post('/guia_nota_venta/anularoficinista', {
      guia: idGuia,
  ...
  anularGuiasSeleccionadas: async (idGuia, idUsuario, motivo) => {
    const response = await api.post('/guia_nota_venta/anularguiasseleccionadas', {
      id_guia: idGuia,
      id_usuario: idUsuario,
      motivoAnulacion: motivo
    });
  ...
  generarPdf: async (idGuia, reimpresoPor = null) => {
    const phpUrl = `${CONFIG.PHP_URL}/guiaNotaVentaPdfImpresion.php?id_guia=${encodeURIComponent(idGuia)}${reimpresoPor ? `&reimpreso_por=${encodeURIComponent(reimpresoPor)}` : ''}`;
  ...
  informacionGuia: async (idGuia) => {
    const response = await api.get('/guia_nota_venta/informacionGuia', { params: { id_guia: idGuia } });
    return response.data;
  },
  ...
  getUltimoDestinatarioPorRemitente: async (cedula) => {
    const response = await api.get('/guia_nota_venta/ultimoDestinatarioPorRemitente', { 
      params: { cedula } 
    });
    return response.data;
  },
  ...
  getGuiasDespacho: async (params = {}) => {
    const response = await api.get('/guia_nota_venta/guialistadoDespacho', { params });
    return response.data;
  },

  // ── Entrega de guía ────────────────────────────────────
  entregarGuia: async (idGuia, nombreOficina, destinoGuia) => {
    const response = await api.post('/guia_nota_venta/updateguiaentregado', {
      id_guia: idGuia,
  ...
  facturarGuia: async (idGuia) => {
    const response = await api.post('/guia_nota_venta/facturarGuia', { id_guia: idGuia });
    return response.data;
  },

  // ── Comprobantes por caja ──────────────────────────────
  getComprobantesPorCaja: async (idGuia, params = {}) => {
    const response = await api.get('/guia_nota_venta/comprobantesxCaja', { 
  ...
  verificarAnulacion: async (idGuia) => {
    const response = await api.get('/guia_nota_venta/verificacionanulacion', { params: { id_guia: idGuia } });
    return response.data;
  },
  ...
  anularGuiasTodas: async (idUsuario, motivo) => {
    const response = await api.post('/guia_nota_venta/anularguiastodas', { id_usuario: idUsuario, motivoAnulacion: motivo });
    return response.data;
  },

  // ── Anular guía como administrador ──
  anularAdministrador: async (idGuia, idUsuario, motivo) => {
    const response = await api.post('/guia_nota_venta/anularadministrador', {
      guia: idGuia,
      id_usuario: idUsuario,
      motivoAnulacion: motivo
    });
  ...
  cancelarAnulacion: async (idGuia, motivo) => {
    const response = await api.post('/guia_nota_venta/cancelarAnulacion', { id_guia: idGuia, motivoAnulacion: motivo });
    return response.data;
  }
};
