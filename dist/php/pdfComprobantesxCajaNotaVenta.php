...
try {
    ...
    $razon_social = !empty($emp['razon_social_empresa']) ? $emp['razon_social_empresa'] : (!empty($emp['nombre_comercial_empresa']) ? $emp['nombre_comercial_empresa'] : '');
    ...
    $rsCaja = mysqli_query($conn, "
        SELECT c.id_caja_nota_venta as id_caja, c.fecha_caja, c.apertura_total_caja, c.cierre_total_caja,
        ...
        WHERE c.id_caja_nota_venta = $idcaja
    ...
    $rsDetalle = mysqli_query($conn, "
        ...
        FROM caja_detalle_nota_venta
        WHERE id_fkcaja_nota_venta = $idcaja
    ...
    $html = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>COMPROBANTES CAJA NOTA DE VENTA</title>
    <style>
    ...
    // ─── LISTADO DE GUÍAS ─────────────────────────────────────────────────────
    $html .= '<div class="section-title">LISTADO DE GUÍAS NOTA DE VENTA</div>';
    $html .= '<table><tr>
        <th width="18%">NRO.</th>
    ...
    // Output inline
    $pdf->Output('ComprobantesNV_'.$idcaja.'.pdf', 'I');
