...
try {
    ...
    if ($id_caja <= 0) {
        throw new Exception("ID de caja nota de venta no válido o no especificado.");
    }
    ...
    $pdfCacheFile = $pdfCacheDir . 'arqueo_caja_notaventa_' . $id_caja . '_t' . md5($tenantIdStr) . '.pdf';
    $noCache = !empty($_GET['nocache']) || !empty($_GET['refresh']);

    if (!$noCache && file_exists($pdfCacheFile) && filesize($pdfCacheFile) > 1000) {
        $fileName = 'ArqueoCajaNotaVenta_' . $id_caja . '.pdf';
    ...
    $sqlCaja = "SELECT c.id_caja_nota_venta as id_caja, c.fecha_caja, CONCAT(u.nombre_usuario, ' ', u.apellido_usuario) as cajero,
    ...
    $cierre_moneda_caja = $cierre_moneda_caja = $caja['cierre_moneda_caja'];
    ...
    // SALDOS (CALL saldoCajaNotaVenta)
    ...
    $pdf->SetTitle('Arqueo de Caja Nota Venta ' . $id_caja);
    ...
    $pdf->Cell($anchoUtil, 5.5, 'ARQUEO DE CAJA NOTAS DE VENTA', 0, 1, 'C');
    $pdf->Ln(2);
