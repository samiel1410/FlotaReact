...
try {
    ...
    $sqlCaja = "SELECT id_caja_nota_venta as id_caja, fecha_caja, CONCAT(nombre_usuario,' ',apellido_usuario) as cajero,
                ...
                FROM caja_nota_venta, usuario 
                WHERE id_caja_nota_venta = $id_caja AND id_fkusuario_caja = usuario.id_usuario";
    ...
    if (mysqli_num_rows($resultCaja) > 0) {
        ...
        // SALDOS (Llamada al stored procedure)
        // Nota: para evitar sync issues con sproc_results, vaciaremos primero o usamos multi_query.
        // Aquí hacemos mysqli_query normal, pero consumimos los results extra.
        $sqlSaldoCaja = "CALL saldoCajaNotaVenta($id_caja)";
        $resultSaldo = mysqli_query($conn, $sqlSaldoCaja) or die(mysqli_error($conn));
        $saldoCaja = mysqli_fetch_array($resultSaldo);
        
        // Limpiar results sobrantes del procedure
        ...
        $totalIngresosDetalle = number_format((float)$saldoCaja['total_ingreso_detalle'], 2, '.', '');
        $totalEgresosDetalle = number_format((float)$saldoCaja['total_egreso_detalle'], 2, '.', '');
        $totalEfectivoDetalle = number_format((float)$saldoCaja['total_efectivo_caja'], 2, '.', '');
        $total_documentos = number_format((float)$saldoCaja['total_documento'], 2, '.', '');
        $estado_cuadre = $saldoCaja['estado_cuadre'];
        $saldo = number_format((float)$saldoCaja['total_diferencia'], 2, '.', '');
        ...
        $total_general_efectivo = number_format((float)$saldoCaja['_total_arqueo'], 2, '.', '');
        $ventas_ingreso = number_format((float)($totalIngresosDetalle + $totalEfectivoDetalle), 2, '.', '');
        
        // DETALLES INGRESOS Y EGRESOS
        $sqlDetalleCajaIngreso = "SELECT id_caja_detalle_nota_venta as id_caja_detalle, fecha_caja_detalle, tipo_caja_detalle, monto_caja_detalle, observacion_caja_detalle, id_fkcaja_nota_venta as id_fkcaja, estado_caja_detalle FROM caja_detalle_nota_venta WHERE tipo_caja_detalle='Ingreso' AND id_fkcaja_nota_venta=$id_caja";
        $resultIngresos = mysqli_query($conn, $sqlDetalleCajaIngreso) or die(mysqli_error($conn));
        
        $sqlDetalleCajaEgreso = "SELECT id_caja_detalle_nota_venta as id_caja_detalle, fecha_caja_detalle, tipo_caja_detalle, monto_caja_detalle, observacion_caja_detalle, id_fkcaja_nota_venta as id_fkcaja, estado_caja_detalle FROM caja_detalle_nota_venta WHERE tipo_caja_detalle='Egreso' AND id_fkcaja_nota_venta=$id_caja";
        ...
        $pdf->SetTitle('Arqueo de Caja ' . $id_caja);
        ...
        $pdf->Output('ArqueoCaja_' . $id_caja . '.pdf', 'I');
        exit;
    } else {
