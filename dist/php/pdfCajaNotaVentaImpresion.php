...
try {
    ...
    // ─── CONSULTA DATOS CAJA NOTA VENTA ──────────────────────────────────────
    $query_caja = "SELECT nombre_sucursal, estado_solicitud, id_caja_nota_venta as id_caja, fecha_caja,
    ...
    if (!$vals_caja) {
        throw new Exception("No se encontró la caja de nota de venta con ID: $id_caja");
    }
    ...
    $pdfCacheFile = $pdfCacheDir . 'caja_nv_' . $id_caja . '_' . $cajaHash . '_t' . md5($tenantIdStr) . '.pdf';
    $noCache = !empty($_GET['nocache']) || !empty($_GET['refresh']);

    if (!$noCache && file_exists($pdfCacheFile) && filesize($pdfCacheFile) > 500) {
        $fileName = 'caja_nota_venta_' . $id_caja . '.pdf';
    ...
    $pdf->Cell($anchoUtil, 4.5, 'CAJA NOTAS DE VENTA', 0, 1, 'C');
    ...
    // ─── 3. SECCIÓN NOTAS DE VENTA (CAJA) ────────────────────────────────────
    $pdf->SetFont('helvetica', 'B', 8.5);
    $pdf->Cell($anchoUtil, 4.2, 'CAJA (NOTAS DE VENTA)', 0, 1, 'C');
    ...
    $pdf->SetFont('helvetica', 'B', 7.5);
    $pdf->Cell($wGuia, 4, 'NOTA VENTA', 'B', 0, 'L');
    ...
    // ─── 4. OTRAS NOTAS DE VENTA ─────────────────────────────────────────────
    if (count($otrasGuias) > 0) {
        $pdf->SetFont('helvetica', 'B', 8);
        $pdf->Cell($anchoUtil, 4, 'OTRAS NOTAS DE VENTA', 0, 1, 'C');
