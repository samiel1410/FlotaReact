...
try {
    ...
    // Consulta principal con JOIN a usuarios para obtener nombre del usuario que recibe y observaciones
    $query = "
        SELECT 
            ...
            gc.fecha_entregada, gc.id_usuario_entregada,
        ...
        LEFT JOIN usuario u ON gc.id_usuario_entregada = u.id_usuario
        WHERE gc.id = $id_guia
        LIMIT 1
    ";
    ...
    // Bloque de entrega con datos reales
    $html .= '<div style="'.$block_style.'">';
    $html .= '<table style="'.$table_style.'" cellpadding="0" cellspacing="0">';
    $html .= '<tr><td style="'.$label_style.'" width="40%">Fecha Entrega:</td><td style="'.$value_style.'">'.($guia['fecha_entregada'] ? date('d/m/Y H:i', strtotime($guia['fecha_entregada'])) : '___________').'</td></tr>';
    $html .= '<tr><td style="'.$label_style.'">Recibe:</td><td style="'.$value_style.'">'.($guia['nombre_cliente'] ? htmlentities($guia['nombre_cliente']) : '__________________________').'</td></tr>';
    $html .= '</table></div>';

    // Línea para firmas SIN CI
    $html .= '<div style="margin-top:8px;margin-bottom:2px;text-align:center;font-size:7pt;">';
    $html .= '<table style="width:100%;"><tr>';
    $html .= '<td style="width:48%;border-top:1px solid #aaa;padding-top:8px;">';
    $html .= ($guia['nombre_cliente'] ? htmlentities($guia['nombre_cliente']) : 'Firma personal');
    $html .= '<br><span style="font-size:6pt;">CI: '.($guia['identificacion_cliente'] ? htmlentities($guia['identificacion_cliente']) : '___________').'</span>';
    $html .= '</td>';
    $html .= '<td style="width:4%;"></td>';
    $html .= '<td style="width:48%;border-top:1px solid #aaa;padding-top:8px;">'.($guia['nombre_usuario'] ? htmlentities($guia['nombre_usuario']) : 'Firma usuario').'</td>';
    $html .= '</tr></table></div>';
    ...
    // MARCA DE AGUA "DESPACHADA"
    if ($guia['estado_guia_companias'] == '1' || $guia['estado_guia_companias'] == 1) {
        $pdf->SetAlpha(0.1); // Opacidad muy baja
        ...
        $pdf->StartTransform();
        $pdf->Rotate(45, $x + 20, $y + 5);
        $pdf->Text($x, $y, 'DESPACHADA');
        $pdf->StopTransform();
    ...
    $pdf->Output('guiaEntregasCompania.pdf', 'I');
