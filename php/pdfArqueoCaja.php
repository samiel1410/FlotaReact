...
try {
    ...
    if ($id_caja <= 0) {
        throw new Exception("ID de caja no válido o no especificado.");
    }
    ...
    $sqlCaja = "SELECT c.id_caja, c.fecha_caja, CONCAT(u.nombre_usuario, ' ', u.apellido_usuario) as cajero,
    ...
    if (mysqli_num_rows($resultCaja) == 0) {
        throw new Exception("No se encontró la caja especificada (#$id_caja).");
    }
    ...
    $pdf->Cell($anchoUtil, 5.5, 'ARQUEO DE CAJA', 0, 1, 'C');
    $pdf->Ln(2);
    ...
    $renderDenominaciones = function($titulo, $subtitulo, $total, $d100, $d50, $d20, $d10, $d5, $d1, $m1, $m50, $m25, $m10, $m05, $m01,
        ...
        // Barra gris encabezado de sección
        ...
        $colLabel = 26;
        $colVal = ($anchoUtil - $colLabel) / 12; // 160 / 12 = 13.33 mm
