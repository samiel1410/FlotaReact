...
try {
    ...
    if ($id_caja <= 0) {
        throw new Exception("ID de caja boletería no válido o no especificado.");
    }
    ...
    $pdfCacheFile = $pdfCacheDir . 'arqueo_caja_boleteria_' . $id_caja . '_t' . md5($tenantIdStr) . '.pdf';
    $noCache = !empty($_GET['nocache']) || !empty($_GET['refresh']);

    if (!$noCache && file_exists($pdfCacheFile) && filesize($pdfCacheFile) > 1000) {
        $fileName = 'ArqueoCajaBoleteria_' . $id_caja . '.pdf';
    ...
    $sqlCaja = "SELECT c.id_caja_boleteria as id_caja, c.fecha_caja, CONCAT(u.nombre_usuario, ' ', u.apellido_usuario) as cajero,
    ...
    $sqlDetalleCajaIngreso = "SELECT id_caja_detalle_boleteria as id_caja_detalle, fecha_caja_detalle, tipo_caja_detalle, monto_caja_detalle, observacion_caja_detalle, id_fkcaja_boleteria as id_fkcaja, estado_caja_detalle FROM caja_detalle_boleteria WHERE tipo_caja_detalle='Ingreso' AND id_fkcaja_boleteria=$id_caja";
    ...
    $sqlDetalleCajaEgreso = "SELECT id_caja_detalle_boleteria as id_caja_detalle, fecha_caja_detalle, tipo_caja_detalle, monto_caja_detalle, observacion_caja_detalle, id_fkcaja_boleteria as id_fkcaja, estado_caja_detalle FROM caja_detalle_boleteria WHERE tipo_caja_detalle='Egreso' AND id_fkcaja_boleteria=$id_caja";
    ...
    $pdf->SetTitle('Arqueo de Caja Boleteria ' . $id_caja);
    ...
    $pdf->Cell($anchoUtil, 5.5, 'ARQUEO DE CAJA BOLETERÍA', 0, 1, 'C');
    $pdf->Ln(2);
