...
try {
    ...
    $sqlCaja = "SELECT id_caja_boleteria, fecha_caja, CONCAT(nombre_usuario,' ',apellido_usuario) as cajero,
                ...
                FROM caja_boleteria, usuario 
                WHERE id_caja_boleteria = $id_caja AND id_fkusuario_caja = usuario.id_usuario";
    ...
    if (mysqli_num_rows($resultCaja) > 0) {
        ...
        <html>
            <head>
                <meta charset="utf-8">
                <title>ARQUEO DE CAJA BOLETERIA</title>
                <style>
                    h1 { color: green; }
                </style>
            </head>
        ...
        $pdf->Output('ArqueoCajaBoleteria_' . $id_caja . '.pdf', 'I');
        exit;
    } else {
