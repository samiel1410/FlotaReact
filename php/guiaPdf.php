...
try {
    ...
    if (empty($vals)) {
        throw new Exception("No se encontró la guía con id = $id_guia");
    }
    ...
    $query_detalles = "SELECT id_detalle_guia, total_tarifa_detalle_guia, cantidad_detalle_guia,
        ...
        FROM detalle_guia WHERE id_fkguia_detalle_envio = $id_guia";
    ...
    $pdf->Cell(60, 6, 'GUÍA DE REMISIÓN', 1, 1, 'C', true);
    $pdf->SetXY(135, 18);
    ...
    $wBox = ($anchoUtil - 4) / 2; // 88 mm cada uno
    $yBoxes = $pdf->GetY();
